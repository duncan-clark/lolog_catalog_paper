...
We were able to fit ERGMs and LOLOG models with the published ERGM terms
but the models did not fit the data well, as noted extensively in
\cite{Goodreau2007}.
...
schoolboys in a class from 1880 to 1881, collected by Johannes Delitsch,
...
The available nodal covariates were: academic class rank, whether the
...
reciprocated friendships. They also concluded the triadic closure
...
included. In addition, the analysis concluded that high academic class
rank students were more likely to have more friendships and that
friendship nominations tended to be hierarchical. That is, pupils tended
to nominate other pupils ranked higher than themselves as friends. There
...
for this network. Introducing rank based ordering i.e., considering
edges involving higher ranked boys first (least academically able)
increases the up-rank effect and produces a highly significant nodal
rank effect. As we are considering high rank boys first, in the
generating process, if all else were equal they become ``filled up'',
i.e., highly connected before the lower rank boys are added. However we
observe in the data low rank boys nominating high rank boys as friends.
To counteract the negative effect on tie formation between low and the
``filled up'' high rank boys, the up-rank effect increases. This
impresses upon us the need to interpret LOLOG fits conditional on the
specified ordering process, in particular when the ordering process is
based on nodal covariates.
...
published fit. The results were not qualitatively different. It seems
likely that the effect due to the omission of the 2-star terms was
absorbed by other terms somewhat.
